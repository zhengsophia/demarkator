...
var Visitors = {
  ...
  CallExpression: ($2, n) => {
    ...
    return name4 === "if" ? $2(args[0]) ? $2(args[1]) : $2(args[2]) : ($2.fn[name4] || Functions[name4]).apply($2.fn, args.map($2));
  },
...
function fieldFilterExpression(predicate, useInRange = true) {
  ...
  if (isFieldEqualPredicate(predicate)) {
    ...
    const { range: range3 } = predicate;
    const lower = isSignalRef(range3) ? { signal: `${range3.signal}[0]` } : range3[0];
    const upper = isSignalRef(range3) ? { signal: `${range3.signal}[1]` } : range3[1];
...
function isFieldRange(range3) {
  return isObject(range3) && "field" in range3;
}
...
function initConfig(specifiedConfig = {}) {
  ...
  const mergedConfig = mergeConfig({}, duplicate(defaultConfig2), font2 ? fontConfig(font2) : {}, color2 ? colorSignalConfig(color2) : {}, fontSize ? fontSizeSignalConfig(fontSize) : {}, restConfig || {});
  if (selection) {
...
var PathOverlayNormalizer = class {
  ...
  run(spec, normParams, normalize2) {
    ...
    const { params, projection, mark, name: name4, encoding: e2, ...outerSpec } = spec;
    ...
    if (selection) {
      return {
        ...
        params: entries(selection).map(([name4, selDef]) => {
          ...
          return { name: name4, value, select, bind };
        })
      };
    }
    return spec;
...
var project = {
  ...
  parse: (model, selCmpt, selDef) => {
    ...
    const signalName = (p, range3) => {
      const suffix = range3 === "visual" ? p.channel : p.field;
  ...
  signals: (model, selCmpt, allSignals) => {
    ...
    const hasSignal = allSignals.filter((s) => s.name === name4);
...
var interval = {
  ...
  signals: (model, selCmpt, signals) => {
    ...
    if (!model.hasProjection) {
      ...
      const update = `unit: ${unitName(model)}, fields: ${name4 + TUPLE_FIELDS}, values`;
...
function pointOrRangePosition(channel, model, { defaultPos, defaultPos2, range: range3 }) {
  if (range3) {
...
var inputBindings = {
  ...
  signals: (model, selCmpt, signals) => {
    ...
    const values3 = proj.items.map((p) => varName(`${name4}_${p.field}`));
...
var legendBindings = {
  ...
  signals: (model, selCmpt, signals) => {
    ...
    const fields = name4 + TUPLE_FIELDS;
    const values3 = proj.items.filter((p) => p.hasLegend).map((p) => varName(`${name4}_${varName(p.field)}_legend`));
    const valid = values3.map((v) => `${v} !== null`).join(" && ");
...
var translate = {
  ...
  signals: (model, selCmpt, signals) => {
    ...
    if (!boundScales) {
      events = events.map((e2) => (e2.between[0].markname = name4 + BRUSH, e2));
    }
    signals.push({
      name: anchor,
      value: {},
...
var zoom = {
  ...
  signals: (model, selCmpt, signals) => {
    ...
    if (!boundScales) {
      events = events.map((e2) => (e2.markname = name4 + BRUSH, e2));
    }
    signals.push({
      name: name4 + ANCHOR2,
      on: [
...
function parseSelectionExtent(model, name4, extent) {
  const vname = varName(name4);
      ...
      if (propIndex) {
        ...
        const signalRef = {
          signal: conditions.map((c) => {
            const { test, ...valueOrSignalCRef } = c;
            return `${expression2(null, test)} ? ${exprFromValueRefOrSignalRef(valueOrSignalCRef)} : `;
...
function sizeSignals(model, sizeType) {
  ...
  if (size === "step") {
    ...
    if (scaleComponent) {
      ...
      const range3 = scaleComponent.get("range");
      if (hasDiscreteDomain(type2) && isVgRangeStep(range3)) {
        const scaleName = model.scaleName(channel);
        if (isFacetModel(model.parent)) {
          const parentResolve = model.parent.component.resolve;
          if (parentResolve.scale[channel] === "independent") {
            return [stepSignal(scaleName, range3)];
          }
        }
        return [
          stepSignal(scaleName, range3),
          {
            name: name4,
...
function stepSignal(scaleName, range3) {
  const name4 = `${scaleName}_step`;
  if (isSignalRef(range3.step)) {
    return { name: name4, update: range3.step.signal };
  } else {
    return { name: name4, value: range3.step };
  }
}
...
function selectedCondition(model, legendCmpt, fieldDef) {
  ...
  return selections.map((name4) => {
    ...
    return `(!length(data(${store})) || (${name4}[${field}] && indexof(${name4}[${field}], datum.value) >= 0))`;
  }).join(" || ");
}
...
var ProjectionComponent = class extends Split {
  constructor(name4, specifiedProjection, size, data) {
    super(
      { ...specifiedProjection },
  ...
  if (nonUnitProjection && mergable) {
    ...
    for (const child of model.children) {
      ...
      if (projection) {
        ...
        child.renameProjection(projection.get("name"), name4);
...
function createBinComponent(t2, bin, model) {
  let as;
  let span;
...
var FacetNode = class extends DataFlowNode {
   ...
   */
  constructor(parent, model, name4, data) {
    super(parent);
    this.model = model;
    this.name = name4;
    this.data = data;
  ...
  getChildIndependentFieldsWithStep() {
    ...
    for (const channel of POSITION_SCALE_CHANNELS) {
      ...
      if (childScaleComponent && !childScaleComponent.merged) {
        ...
        const range3 = childScaleComponent.get("range");
        if (hasDiscreteDomain(type2) && isVgRangeStep(range3)) {
...
function getImplicitFromSelection(model) {
  ...
  if (isUnitModel(model) && model.component.selection) {
    for (const name4 of keys(model.component.selection)) {
...
function parseSelectionDomain(model, channel) {
  ...
  const bin = (_a2 = model.fieldDef(channel)) == null ? void 0 : _a2.bin;
...
function assembleScalesForModel(model) {
  return keys(model.component.scales).reduce((scales, channel) => {
    ...
    const { name: name4, type: type2, selectionExtent, domains: _d2, range: _r2, reverse: reverse2, ...otherScaleProps } = scale;
    const range3 = assembleScaleRange(scale.range, name4, channel, model);
  ...
  for (const property of RANGE_PROPERTIES) {
    if (specifiedScale[property] !== void 0) {
      ...
      if (!supportedByScaleType) {
        ...
        switch (property) {
          case "range": {
            const range3 = specifiedScale.range;
            if (isArray(range3)) {
              if (isXorY(channel)) {
                return makeExplicit(range3.map((v) => {
                  if (v === "width" || v === "height") {
...
function parseFacetHeader(model, channel) {
  ...
  if (model.channelHasField(channel)) {
    ...
    const labels3 = fieldDef.header !== null ? getFirstDefined((_a2 = fieldDef.header) == null ? void 0 : _a2.labels, config.header.labels, true) : false;
...
function defaultUnitSize(model, sizeType) {
  ...
  if (scaleComponent) {
    ...
    if (hasDiscreteDomain(scaleType2)) {
      ...
      if (isVgRangeStep(range3) || isStep(size)) {
        return "step";
      } else {
        return size;
      }
...
var FacetModel = class _FacetModel extends ModelWithField {
  ...
  assembleFacet() {
    const { name: name4, data } = this.component.data.facetRoot;
  ...
  class Comparator {
    ...
    test(version5) {
      ...
      if (typeof version5 === "string") {
        try {
          version5 = new SemVer3(version5, this.options);
        } catch (er) {
          return false;
        }
      }
      return cmp2(version5, this.operator, this.semver, this.options);
    }
...
function requireRange() {
  ...
  const testSet = (set2, version5, options) => {
...
var satisfies = (version5, range3, options) => {
  try {
    range3 = new Range(range3, options);
  } catch (er) {
    return false;
  }
  return range3.test(version5);
};
...
async function embed(el, spec, opts = {}) {
  ...
  const mergedOpts = _objectSpread(_objectSpread({}, mergeDeep2(parsedOpts, usermetaOpts)), {}, {
